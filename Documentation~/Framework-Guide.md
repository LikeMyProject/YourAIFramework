...
在无头环境跑测试——只要不调 Pump，世界就是静止的。这也是框架 407 项离线断言
能够存在的原因：所有逻辑不碰引擎就能测。
...
对应离线用例组：`stat/*`（tools/core-test，407 项断言的一部分）。
...
对应离线用例组：`catalog/*`（tools/core-test，407 项断言的一部分）。
...
**质量保障**：四道门禁 —— 407 项离线断言（纯 BCL 跑，不进编辑器）、引擎编译门禁
