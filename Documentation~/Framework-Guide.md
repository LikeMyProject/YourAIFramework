...
YooAsset / Unity Localization / HybridCLR 是**可选**集成，装了包框架自动启用对应
适配层，没装也完全不影响。
...
在无头环境跑测试——只要不调 Pump，世界就是静止的。这也是框架 213 项离线断言
能够存在的原因：所有逻辑不碰引擎就能测。
...
| 资源 | `AssetManager` | 统一加载入口，后端可换（含 YooAsset） | 是 |
...
- **生命周期两段式**：`Opening → Open → Closing → Closed`，
  ...
  将来接入 YooAsset 异步加载 UI 预制体时接口不变）；
...
## 10. 资源管理 AssetManager

**干什么**：全工程统一的资源加载入口。你只面对 `AssetManager`，后端随便换——
本地直读、YooAsset、Addressables 都只是 Provider。
...
var assets = new AssetManager(new IAssetProvider[] {
    ...
    // 装了 YooAsset 还可以插：new YooAssetProvider(package, "DefaultPackage")
});
...
Load = 整表替换（热更语义）；装了 Unity Localization 包可 `AddSource(new
UnityLocalizationSource("MyTable"))` 并表使用。
...
var audio = new AudioManager(new UnityAudioBackend(root, 16,
    key => /* 从 AssetManager/YooAsset 拿 AudioClip */ null));
...
**干什么**：版本检查 → 补丁下载 → 应用 → 程序集重载的**编排器**。框架引入不自研：
下载能力复用 YooAsset，代码热更复用 HybridCLR，它们以"步骤"形态插进编排。
...
flow.Run(new IHotUpdateStep[] {
    new YooAssetCheckVersionStep(...),               // 引擎侧步骤（装 YooAsset 后）
    new YooAssetDownloadStep(...),
    new HybridClrReloadStep(name => loadBytes(name)) // 装了 HybridCLR 才有
});
...
│   │   ├── Data/   Assets/       #   DataTable / AssetManager
│   │   ├── Localization/ Audio/  #   LocalizationStore / AudioManager
...
│                                   #   YooAsset·UnityLocalization·HybridCLR 适配层
├── Editor/                       # AI 设置窗口
├── Documentation~/               # 本文档 + YourAI-Guide.md
└── Samples~/                     # 模块速览 Demo（16 个，导入即跑）/ 聊天示例
```

**质量保障**：213 项离线断言（纯 BCL 跑，不进编辑器）+ 引擎编译门禁（全部源码链
Unity 6.5.8f1 真实程序集编译，API 改名立刻红）。每个模块的用例组按前缀组织：
`ui/*`、`data/*`、`asset/*`、`loc/*`、`audio/*`、`net/*`、`entity/*`、`sdk/*`、
`diag/*`、`hot/*`。
