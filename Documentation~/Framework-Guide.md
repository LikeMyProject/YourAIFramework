...
21. [AI 模块（可选）](#21-ai-模块可选)
22. [完整引导示例（可直接抄）](#22-完整引导示例可直接抄)
23. [框架如何对待你的错误（错误姿态总表）](#23-框架如何对待你的错误错误姿态总表)
24. [目录结构与测试](#24-目录结构与测试)
25. [FAQ](#25-faq)
...
在无头环境跑测试——只要不调 Pump，世界就是静止的。这也是框架 370 项离线断言
能够存在的原因：所有逻辑不碰引擎就能测。
...
└── Samples~/                     # 模块速览 Demo（22 个，导入即跑）/ 聊天示例
```

**质量保障**：四道门禁 —— 370 项离线断言（纯 BCL 跑，不进编辑器）、引擎编译门禁
（全部源码链 Unity 6.5.8f1 真实程序集编译，API 改名立刻红）、Demo 编译门禁（二十二个
