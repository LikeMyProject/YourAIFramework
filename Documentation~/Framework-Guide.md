...
24. [框架如何对待你的错误（错误姿态总表）](#24-框架如何对待你的错误错误姿态总表)
...
在无头环境跑测试——只要不调 Pump，世界就是静止的。这也是框架 406 项离线断言
能够存在的原因：所有逻辑不碰引擎就能测。
...
| 装载顺序 | `AssemblyLoadOrder` | root 名 → 依赖闭包，按拓扑序填列表（被依赖者先；成环报出环上的名字） |
...
- **每帧一个程序集**：单帧工作量有界，热更不会把加载界面冻住一帧；顺序来自清单里的依赖，
  不是人工排的名单；
- **失败是值贯穿到底**：文件缺失、声明长度不符、CRC32 不符、装载器拒绝（IL2CPP 上的
...
| 持续效果 | `StatEffect` / `EffectHost` | 泵驱动的 buff/debuff 状态机，到期精确摘除 |
| 伤害管道 | `DamageProfile` / `DamagePipeline` | 攻守两张表 → 一个整数伤害，失败是值 |
...
3. **写严读松**：写入未声明的属性当场抛（属性名拼错在装配那一刻就爆），读取未声明的
   属性返回 0（配表驱动的属性集合本来就会动态长出来，读侧报错会把正常流程也打断）。
...
| 策略 | 语义 | 典型用法 |
|---|---|---|
| `Refresh` | 已在则只刷新时长，层数恒为 1 | 减速、增益（否则两个法师各放一次就把玩家钉死） |
...
2. **随机消费次数由配置决定**：暴击率 > 0 才抽暴击、浮动 > 0 才抽浮动，抽了几次如实记在
   ...
   结果。`Amount` 是夹取后的整数，`Raw` 是夹取前的真值，封顶不会把真实强度藏起来；
...
对应离线用例组：`stat/*`（tools/core-test，406 项断言的一部分）。
...
解析不到**不是异常**：地址可能本就不该存在（某语言没有配音）。返回 `Resolved == false`，由调用方与校验器决定要不要当问题 —— 与“属性缺失是值”是同一条红线。
...
对应离线用例组：`catalog/*`（tools/core-test，406 项断言的一部分）。
...
### 20.1 分工：内核管接缝，适配层管引擎

- **内核**（`YourFramework.Foundation/Script/`）只定义接缝 —— 代码块、清单、装载序、沙箱策略、
  失败姿态。**它不认识任何脚本引擎**，也不依赖任何第三方包，所以它能在那道离线断言门禁里
  被逐条验证。
- **适配层**（`Runtime/YourFramework.XLua/`）把 XLua 接到接缝上，是**可选程序集**：
...
| 接缝 | 一句话 | 关键纪律 |
|---|---|---|
| `IScriptRuntime` | 能装块、能卸块、能调入口、能被逐帧泵 | 内容问题失败是值；写错代码（null 块、已关停）抛 |
| `IScriptChunkSource` | 代码块的文本来源 | 严格 UTF-8（坏字节报出来，不变成替换符）+ 路径闸 |
...
对应离线用例组：`script/*`。适配层另有自己的门禁（见第 24 节）。

### 20.5 边界说清楚

- **XLua 不在框架包内，也不会有**：框架的红线是零第三方包依赖。所以形态是「可选接缝 + 一个实现」，
  用不用、用哪个版本由你决定；
- **真实 XLua 的运行时行为需要你在 Unity 里验一次**：离线门禁能证的是「适配层对着冻结的 XLua
...
| 资源在**起手时**扣 | 吟唱被打断 = 资源白花。语义明确、可断言，而不是"看情况" |
...
能在无头环境逐条断言，也能整块换成另一种落地策略（例如"特效先行、伤害后结算"）。
...
│   │   ├── Script/               #   脚本层接缝：代码块 / 清单 / 装载序 / 沙箱策略 / 补丁阶段
...
│   └── YourFramework.XLua/         # 可选适配层：XLua → 脚本层接缝
...
**质量保障**：四道门禁 —— 406 项离线断言（纯 BCL 跑，不进编辑器）、引擎编译门禁
（全部源码链 Unity 6.5.8f1 真实程序集编译，API 改名立刻红）、Demo 编译门禁（二十三个
演示全量编译）、以及 XLua 适配层门禁（`tools/xlua-adapter`：把适配层依赖的 XLua 公开 API
子集冻结成录制式替身，编译适配层并驱动一遍，验证构造/装载/卸块/入口/泵/关停的调用协议
与失败折算）。

门禁的边界也写清楚：XLua 那道证的是**适配层的协议**，不是 **XLua 自己的运行时行为** ——
...
**Q：框架内置 Lua / XLua 吗？**
A：不内置。内核只提供脚本层接缝（代码块 / 清单 / 装载序 / 沙箱 / 失败姿态，零第三方依赖），
