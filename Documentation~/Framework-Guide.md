...
19. [AI 模块（可选）](#19-ai-模块可选)
20. [完整引导示例（可直接抄）](#20-完整引导示例可直接抄)
21. [框架如何对待你的错误（错误姿态总表）](#21-框架如何对待你的错误错误姿态总表)
22. [目录结构与测试](#22-目录结构与测试)
23. [FAQ](#23-faq)
...
在无头环境跑测试——只要不调 Pump，世界就是静止的。这也是框架 342 项离线断言
能够存在的原因：所有逻辑不碰引擎就能测。
...
对应离线用例组：`stat/*`（tools/core-test，342 项断言的一部分）。
...
对应离线用例组：`catalog/*`（tools/core-test，342 项断言的一部分）。
...
└── Samples~/                     # 模块速览 Demo（21 个，导入即跑）/ 聊天示例
```

**质量保障**：三道门禁 —— 342 项离线断言（纯 BCL 跑，不进编辑器）、引擎编译门禁
（全部源码链 Unity 6.5.8f1 真实程序集编译，API 改名立刻红）、Demo 编译门禁（二十一个
演示全量编译）。每个模块的用例组按前缀组织：`ui/*`、`data/*`、`asset/*`、`pack/*`、
`loc/*`、`locale/*`、`plural/*`、`audio/*`、`net/*`、`entity/*`、`sdk/*`、`diag/*`、`hot/*`、`asm/*`、`stat/*`、`catalog/*`。
