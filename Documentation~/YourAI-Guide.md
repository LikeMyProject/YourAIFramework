...
对应离线用例组：`asset/*` 与 `pack/*`（tools/core-test，406 项断言的一部分）。
...
对应离线用例组：`loc/*` 与 `locale/*`、`plural/*`（tools/core-test，406 项断言的一部分）。
...
对应离线用例组：`net/*`（tools/core-test，406 项断言的一部分；ClientWebSocketLink
...
对应离线用例组：`entity/*`、`sdk/*`、`diag/*`（tools/core-test，406 项断言的一部分）。
...
对应离线用例组：`hot/*`、`asm/*`（tools/core-test，406 项断言的一部分）。
...
不引用引擎，也不引用数值层）；离线断言在 `tools/core-test/SkillCases.cs`（`skill/*` 组，36 项）；
