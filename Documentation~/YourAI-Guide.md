...
- `DataTableSet.Load(name, table)`——同表名再 Load = **整表替换，即数值热更落点**（YooAsset 热更管线落地后直接对接）；`Get(name)` 缺表返回 null，是否致命由调用方决定。
...
AssetManager，YooAsset / Addressables / 本地直读都只是 Provider 实现，随时可换。
...
**YooAsset 集成（可选，引入不自研）**：`YooAssetProvider`（YourFramework.UnityRuntime）
把 YooAsset 资源包包进自家 Provider。编译由 asmdef `versionDefines` 守卫：宿主工程装了
`com.tuyoogame.yooasset` 2.3+ 时自动定义 `YOURAI_YOOASSET`，适配层生效；没装包则干净
编译为空，框架其余部分照常工作。key = YooAsset location，句柄随请求轮询，
`LastError` 原文进 `Error`。
...
var assets = new AssetManager(new IAssetProvider[] {
    ...
    new YooAssetProvider(yooPackage, "DefaultPackage"), // 装了 YooAsset 才有
...
对应离线用例组：`asset/*`（tools/core-test，180 项断言的一部分）。

### A.6 本地化（YourFramework.Localization）

`LocalizationStore` / `ILocalizationSource` —— 自家取词链在前、引擎后端为可选插件：
游戏代码只面对一个 Get，Unity Localization 装不装、用不用，调用方无感。
...
**Unity Localization 桥（可选，引入不自研）**：`UnityLocalizationSource`
（YourFramework.UnityRuntime）挂在源插槽上，表未预载/语言不符一律让路。编译由
asmdef `versionDefines` 守卫：宿主装了 `com.unity.localization` 1.5+ 自动定义
`YOURAI_UNITY_LOCALIZATION`；没装包该文件干净编译为空（与 YooAsset 适配同一纪律）。
...
// 装了 Unity Localization 且想并表使用：
loc.AddSource(new UnityLocalizationSource("MyTable"));
```

对应离线用例组：`loc/*`（tools/core-test，188 项断言的一部分）。
...
**下载能力（决策记录）**：资源下载不另起网络模块 —— YooAsset 的下载器已覆盖
断点续传/校验/重试/限速/多线程，M3-2 的 YooAssetProvider 即唯一落点；本模块只管
长连接游戏业务流量。两条线职责不重叠。

对应离线用例组：`net/*`（tools/core-test，202 项断言的一部分；ClientWebSocketLink
...
`HotUpdateFlow` / `IHotUpdateStep` —— 引入不自研的最终落点：编排只管顺序、进度
事件、失败截停与终态语义；YooAsset / HybridCLR 的能力以"步骤"形态插入。
...
- **共享上下文**：`flow.Context.Set/Get` 传递版本号、名单、产物（如 HybridCLR
  加载完的程序集表）；
...
**HybridCLR 适配（可选，`#if YOURAI_HYBRIDCLR`）**：`HybridClrReloadStep`
（YourFramework.UnityRuntime）—— 每帧加载一个程序集（有界工作量），先补 AOT
元数据再按依赖序加载热更 DLL；字节由宿主注入的加载器提供（接 AssetManager/
YooAsset 步骤的产物）。versionDefines：装了 `com.code-philosophy.hybridclr`
1.12+ 自动启用；没装包干净编译为空。
...
flow.Run(new IHotUpdateStep[] {
    new YooAssetCheckVersionStep(...),      // YooAsset 适配（引擎侧）
    new YooAssetDownloadStep(...),          // 断点续传/校验由 YooAsset 负责
    new HybridClrReloadStep(bytes => ...),  // 装了 HybridCLR 才有
});
GameEntry.Ensure().Register(flow);
```

对应离线用例组：`hot/*`（tools/core-test，213 项断言的一部分）。
