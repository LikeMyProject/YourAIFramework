...
7. [调试与诊断](#7-调试与诊断)
8. [验证与测试](#8-验证与测试)
9. [设计必须守的规矩（先读，能省一半时间）](#9-设计必须守的规矩)
10. [目录结构](#10-目录结构)
...
| **失败也是值，不是异常** | 网络断了、Key 错了、模型罢工——都返回带原因的结果对象 | 你不用 try/catch 包游戏逻辑；断网时 NPC 给出"理由"，游戏照跑 |
...
| **内核零 Unity 依赖** | Core/Memory/Grounding/Agent/Presentation 是纯 C#（netstandard2.1） | 可在控制台程序里跑 135 条离线检查，改动不会到了真机才炸 |
...
| `FileMemoryStore(directory)` | JSON 写进文件（建议放 `Application.persistentDataPath` 下），重启不忘 |
...
| **Window → YourAI → Smoke Test (Offline)** | 一键跑 6 项确定性冒烟（降级、回合生命周期、并行工具、ViewModel 事件、团队接力、记忆往返），全部通过说明装好了 |
...
| 请求失败为什么 | `turn.Result.Error` + `turn.Result.HttpStatus`（失败也是值，永远有原因） |
...
框架自带**双离线检查**（改完代码各跑一次，全部通过才提交）：

| 提交前检查 | 位置 | 内容 |
|---|---|---|
| `core-test` | `tools/core-test` | 135 条检查项：契约、记忆、装配、回合、团队、ViewModel、诊断 |
...
仓库根目录另有 `DESIGN-ANALYSIS.md`（设计分析与决策记录）、`SMOKE-TEST.md`（真机验收清单）、`tools/`（离线检查与 mock 服务器）。

---

## 附录 A：通用内核（YourFramework.Foundation）

除 AI 七层外，框架自带业务中立的通用内核（游戏 / XR / 仿真 / 孪生通用），同一套纪律：纯 C# 内核 + 引擎薄壳、双离线检查、失败也是值。
...
if (error != null) ui.ShowLoadError(error);   // 失败也是值
```

对应离线用例组：`module/*`、`event/*`、`pool/*`、`save/*`、`setting/*`（tools/core-test，154 项检查项的一部分）。

### A.1 流程状态机（YourFramework.Flow）

游戏生命周期骨架（启动 → 检查更新 → 登录 → 主菜单 → 战斗……），M2 交付：

- `ProcedureFlow : IModule`——注册进 `ModuleCenter` 即被统一逐帧驱动，无需第二个循环；
...
对应离线用例组：`flow/*`（tools/core-test，159 项检查项的一部分）。

### A.2 UI 面板栈（YourFramework.UI 内核）

面板系统（PanelStack/UIPanelManager）自研实现，M2 交付（纯内核已走完整个流程，PanelRenderer 适配壳随后）：
...
对应离线用例组：`ui/*` + `module/pump-error-isolation`（tools/core-test，166 项检查项的一部分）。
...
对应离线用例组：`aimodule/*`（tools/core-test，169 项检查项的一部分）。
...
对应离线用例组：`data/*`（tools/core-test，174 项检查项的一部分）。
...
- `AssetManager.Load(key)` → `AssetRequest`：同 key 全程唯一（去重 + Ready 缓存一体）；
  ...
  的 Provider，轮询直到落定；**轮询逐帧驱动，无 Task/回调风暴**；
- `AssetRequest` 粘性终态 `Ready / Failed`：失败带原因（`Error`），不抛异常 —— 运行期
  I/O 故障一律"失败也是值"；`Load("")` 这类参数错误才 fail-fast；
...
（清单 + 版本 + 下载队列 + 缓存记录）包进同一个 Provider 契约：地址 → 依赖全链（拓扑序）
...
对应离线用例组：`asset/*` 与 `pack/*`（tools/core-test，407 项检查项的一部分）。
...
对应离线用例组：`loc/*` 与 `locale/*`、`plural/*`（tools/core-test，407 项检查项的一部分）。
...
- **回收**：非循环声部自然结束后由后端在 Pump 时上报（isPlaying 轮询引擎实际内容），
...
对应离线用例组：`audio/*`（tools/core-test，195 项检查项的一部分）。
...
- **状态机**：`Idle → Connecting → Open ⇄ Reconnecting → Closed`；全部时间由
  ...
  快进测试天然支持；
- **必须守的规矩四条全落实**：超时/心跳/重试/退避全部在 `NetClientConfig` 显式可配
...
对应离线用例组：`net/*`（tools/core-test，407 项检查项的一部分；ClientWebSocketLink
为编译提交前检查级验证，活链路联调在真机/本地回环进行）。

### A.9 实体 / SDK 管道 / 诊断台（M5 三件套）

**实体注册表（YourFramework.Entity / `EntityRegistry`）**：NPC、机关、子弹、系统
组件的统一管理 —— `Spawn` 进门（`ISpawnable.OnSpawn`）、每帧 `EntityPump(delta)`
...
对应离线用例组：`entity/*`、`sdk/*`、`diag/*`（tools/core-test，407 项检查项的一部分）。
...
对应离线用例组：`hot/*`、`asm/*`（tools/core-test，407 项检查项的一部分）。
...
自己的组件里持有一张 `StatSheet`（模块间不互相引用这条必须守的规矩照旧）。
...
else { Debug.Log(hit.Outcome); }            // 缺攻方属性 / 无伤害 —— 失败也是值
```
...
游戏侧要热更代码时，程序集重载在 IL2CPP 上会被平台拒绝，而**解释执行不会被拒绝** —— 于是框架把「热更代码」这件事在 IL2CPP 上的落点做成了脚本层：内核定义预留接口（代码块 / 清单 / 装载序 / 沙箱策略 / 失败时的处理，零第三方依赖），XLua 是一个可插拔实现。
...
3. **`require` 走我们自己的来源**：补丁代码里的 `require("core.util")` 由 `LuaEnv.AddLoader` 接到 `IScriptChunkSource`，并套同一道路径检查（拒绝绝对路径与「..」）—— 请求名来自下载来的代码，属于不可信输入。

对应离线用例组：`script/*`（内核）；适配层另有提交前检查 `tools/xlua-adapter`。详见 `Framework-Guide.md` 第 20 节。

---

### A.14 技能系统（YourFramework.Skills）

**目标**：把"加一个技能要改代码、改包、重新出客户端"变成"加一段 JSON"。

**为什么内核不认识数值系统**：技能流程（冷却、吟唱、目标选择、效果分发）与"伤害怎么算"
是两件会分别变化的事 —— 前者被项目结构影响（快照从哪来、谁算活着），后者被策划天天调
（公式、减伤、暴击）。绑在一起就等于每次调数值都要动技能流程的代码。所以内核只认两个接口：
`ISkillStatSource`（读属性 + 扣资源）与 `ISkillEffectSink`（把一条效果落到一个目标）。
`StatSheetStatSource` / `StatSkillEffectSink` 是接数值系统的那套实现，
**整个模块里只有它们知道 StatSheet / DamagePipeline / EffectHost 存在**。
这条边界不靠注释维持：离线检查用反射扫内核类型，确认它们不引用数值层的任何类型。

**为什么"资源在起手时扣"**：这是唯一能让"吟唱被打断"有确定语义的选择。落地才扣，
打断就等于没发生，玩家会开始用取消吟唱来白嫖前摇；起手就扣，"打断 = 资源白花"
就是一条明确的、可检查项的规则，而不是看情况。

**为什么落地失败不截停**：一条技能可能有两三条效果（伤害 + 挂 debuff），目标可能已经死了。
半途停下会让"充能已消耗、但 debuff 没上"变成一个谁也说不清的状态；继续走完、把每条效果
的结果如实报成事件，排查时看事件流水就够了。同理，热更钩子失败只报 `HookFailed`，
不截停施法 —— 脚本是热更代码，一个错别字不该让技能整体失灵。

**为什么配置错要 fail-fast**：技能表是构建期产物，坏表的症状是"这个技能打不出伤害"——
几乎不可能被玩家或测试报出来；前置成环更隐蔽（技能永远灰着）。所以坏 JSON、未知枚举、
负数时长、算不出数的效果、成环，全部在装载期抛，报错带上行号。

**技能树为什么用三态标记**：两态（访问过 / 没访问过）无法区分"环"和"两个技能共用一个
公共前置"。后者是合法的技能树形状，两态实现会把一张好表判死。

**热更钩子为什么是入口名字符串**：技能表要能不改代码就换表现，配置里就只能写"调用哪个入口"，
而不是代码引用。代价是钩子可能不存在（脚本写错、补丁没装）。

**表的位置**：`Runtime/YourFramework.Foundation/Skills/`（六个文件，全在纯 C# 内核里，
不引用引擎，也不引用数值层）；离线检查在 `tools/core-test/SkillCases.cs`（`skill/*` 组，36 项）；
演示见 Module Demos 的 Demo 23。
