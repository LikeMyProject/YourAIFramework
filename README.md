...
暂停、快进、回放；**273 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
...
| Module Demos（从这里开始） | 十九个模块速览演示，导入即跑、零资产依赖 | Package Manager → Samples → Import，[导览表](Samples~/ModuleDemos/README.md) |
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **273 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
| `transport-compile` | 全部源码链接 Unity 6.5.8f1 真实程序集编译，抓引擎 API 漂移 | `cd tools/transport-compile && dotnet build` |
| `demo-compile` | 十九个 Demo 全量编译验证 | `cd tools/demo-compile && dotnet build` |
...
3. 按 Play——Console 依次上演 **19 个模块演示**（每个模块一个，自包含零资产依赖），
   运行中可右键组件名单独重放任意一个。
