...
暂停、快进、回放；**213 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
无头 CI 全覆盖；YooAsset / Unity Localization / HybridCLR 装包自动启用、不装干净编译；
...
graph TB
    ...
    subgraph OPS["运维层"]
        ...
        H["HotUpdateFlow 热更编排（YooAsset+HybridCLR）"]
    end
...
3. **可选集成零污染**——YooAsset / Unity Localization / HybridCLR 由 asmdef
   `versionDefines` 守卫，装包自动启用适配层，不装干净编译。
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **213 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
| `transport-compile` | 全部源码链接 Unity 6.5.8f1 真实程序集编译，抓引擎 API 漂移 | `cd tools/transport-compile && dotnet build` |
| `demo-compile` | 十六个 Demo 全量编译验证 | `cd tools/demo-compile && dotnet build` |
...
| 包 | 启用方式 | 适配层 |
|---|---|---|
| [YooAsset](https://github.com/tuyoogame/YooAsset) 2.3+ | 装包即自动（`YOURAI_YOOASSET`） | 资源 Provider + 热更下载步骤 |
| [Unity Localization](https://docs.unity3d.com/Packages/com.unity.localization@latest) 1.5+ | 装包即自动（`YOURAI_UNITY_LOCALIZATION`） | 本地化外部源 |
| [HybridCLR](https://github.com/focus-creative-games/hybridclr) 1.12+ | 装包即自动（`YOURAI_HYBRIDCLR`） | 程序集重载步骤 |

不装任何一个包，框架照常编译运行——可选集成由 asmdef `versionDefines` 守卫。
...
| **内容** | `DataTable` / `AssetManager` / `LocalizationStore` | 数据表 · 资源（YooAsset 可选）· 本地化 |
...
| **运维** | `FrameworkDiagnostics` / `HotUpdateFlow` | 诊断台（错误环+快照）· 热更编排（YooAsset+HybridCLR） |
...
3. 按 Play——Console 依次上演 **16 个模块演示**（每个模块一个，自包含零资产依赖），
   运行中可右键组件名单独重放任意一个。
