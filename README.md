...
Version 0.1.12。本版是实机巡演后的一轮修正：把演示里三处"跑不到位"的地方改对了 ——
流程机演示少了启动那一下（Init，之前"什么都没演就结束"）、热更程序集演示的清单填了
占位校验值（三段完整性故事全被它挡住）、下载替身按固定字节数上报（和清单对不上）；
另有两处文案和一处存档返回值的说明改得更诚实。内核与全部模块的离线检查全部通过；
引擎侧（编辑器导入、真机发声、真实链路联调）正在实机验收。
