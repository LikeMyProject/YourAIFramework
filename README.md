...
暂停、快进、回放；**370 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
...
| Module Demos（从这里开始） | 二十二个模块速览演示，导入即跑、零资产依赖 | Package Manager → Samples → Import，[导览表](Samples~/ModuleDemos/README.md) |
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **370 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
| `transport-compile` | 全部源码链接 Unity 6.5.8f1 真实程序集编译，抓引擎 API 漂移 | `cd tools/transport-compile && dotnet build` |
| `demo-compile` | 二十二个 Demo 全量编译验证 | `cd tools/demo-compile && dotnet build` |
...
| **内容** | `DataTable` / `AssetManager` + 自研资源包 / `LocalizationStore` + 自研本地化包 / `AddressCatalog` | 数据表 · 资源（清单/版本/下载/缓存）· 本地化（语言链/多表/CLDR 复数/资源地址）· 地址目录（变体/补丁/校验） |
...
Version 0.1.5。内核与全部模块的离线门禁全绿；引擎侧（编辑器导入、真机发声、
活链路联调）处于实机验收进行时。
...
3. 按 Play——Console 依次上演 **22 个模块演示**（每个模块一个，自包含零资产依赖），
   运行中可右键组件名单独重放任意一个。
