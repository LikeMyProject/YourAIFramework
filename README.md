...
暂停、快进、回放；**407 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **407 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
...
Version 0.1.9。内核与全部模块的离线门禁全绿；引擎侧（编辑器导入、真机发声、
活链路联调）处于实机验收进行时。
