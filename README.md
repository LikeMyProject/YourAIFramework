...
十八个模块**用哪个注册哪个**，公开 API **无 async/await**，时间由宿主注入，一切皆可
暂停、快进、回放；**406 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
...
场景里挂一个脚本，运行即活——这就是接入全部十八个模块所需的全部代码：
...
| 资源 | 说明 | 入口 |
|---|---|---|
| YourAIFramework 本体 | 十八模块 + 可选 AI 七层，包内自含全部源码与文档 | [本仓库](.) |
| Module Demos（从这里开始） | 二十三个模块速览演示，导入即跑、零资产依赖 | Package Manager → Samples → Import，[导览表](Samples~/ModuleDemos/README.md) |
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **406 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
| `transport-compile` | 全部源码链接 Unity 6.5.8f1 真实程序集编译，抓引擎 API 漂移 | `cd tools/transport-compile && dotnet build` |
| `demo-compile` | 二十三个 Demo 全量编译验证 | `cd tools/demo-compile && dotnet build` |
| `xlua-adapter` | XLua 适配层：对着冻结的公开 API 子集编译 + 协议断言（21 项） | `cd tools/xlua-adapter && dotnet run` |
...
| **内容** | `DataTable` / `AssetManager` + 自研资源包 / `LocalizationStore` + 自研本地化包 / `AddressCatalog` / `SkillLibrary` + `SkillBook` | 数据表 · 资源（清单/版本/下载/缓存）· 本地化（语言链/多表/CLDR 复数/资源地址）· 地址目录（变体/补丁/校验）· 技能（表驱动/目标选择/冷却充能/施法流程） |
| **表现** | `PanelStack` + `UIPanelManager` / `AudioManager` | UI 面板栈（PanelRenderer）· 声音分组混音 |
| **流程** | `ProcedureFlow` / `EntityRegistry` / `StatSheet` + `EffectHost` + `DamagePipeline` | 启动/游戏状态机 · 实体统一住址 · 数值（属性/持续效果/伤害） |
...
Version 0.1.8。内核与全部模块的离线门禁全绿；引擎侧（编辑器导入、真机发声、
活链路联调）处于实机验收进行时。
...
3. 按 Play——Console 依次上演 **23 个模块演示**（每个模块一个，自包含零资产依赖），
   运行中可右键组件名单独重放任意一个。
