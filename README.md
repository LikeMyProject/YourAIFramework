...
暂停、快进、回放；**220 项离线断言** `dotnet run` 就能跑，不进编辑器、不要 License、
...
| 门禁 | 说明 | 运行 |
|---|---|---|
| `core-test` | 纯 .NET SDK 跑 **220 项断言**，无头、无编辑器、不要 License | `cd tools/core-test && dotnet run` |
...
| **内容** | `DataTable` / `AssetManager` + 自研资源包 / `LocalizationStore` | 数据表 · 资源（清单/版本/下载/缓存）· 本地化 |
...
3. 按 Play——Console 依次上演 **17 个模块演示**（每个模块一个，自包含零资产依赖），
   运行中可右键组件名单独重放任意一个。
