...
namespace YourAIFramework.ModuleDemos
{
    ...
    public static class DemoServices
    {
        ...
        public static IEnumerator Diagnostics()
        {
            ...
            diag.Set("fps", 59.9);                  // 仪表（由调用方喂值 —— 这里的 59.9 是演示数字，不是实测帧率）
            diag.RecordError("演示：这是一条被诊断台捕获的错误");

            Debug.Log("[Demo14] items.sold=" + diag.GetValue("items.sold")
                + "，fps=" + diag.GetValue("fps") + "（仪表是调用方喂的演示值，不是实测帧率）"
        ...
        public static IEnumerator HotUpdate()
        {
            ...
            yield return PumpHotUntilDone(flow, "第一轮");
            ...
            yield return PumpHotUntilDone(flow, "第二轮");
            Debug.Log("[Demo15] 失败原因已通过 OnFailed(阶段名, 原因) 交给 UI 有话可说。");
        }

        /// <summary>
        /// 把热更流程泵到终态：每帧 Pump 一次。本地/已注册实例都要有人泵 ——
        /// 这里显式泵是为了让"逐帧推进"看得见，上限只为"万一不结束也别死等"
        /// （三个假阶段合计约 130 帧就该走完）。
        /// </summary>
        private static IEnumerator PumpHotUntilDone(HotUpdateFlow flow, string round, int maxFrames = 900)
        {
            int frames = 0;
            while (flow.IsRunning && frames < maxFrames)
            {
                flow.Pump();
                frames++;
                yield return null;
            ...
            if (flow.IsRunning)
            {
                Debug.LogWarning("[Demo15] " + round + "：泵了 " + frames + " 帧流程仍在跑 —— "
                    + "每个阶段都该自己推进到终态，没结束说明某个阶段的 Pump 一直返回 Running。");
            }
        }
