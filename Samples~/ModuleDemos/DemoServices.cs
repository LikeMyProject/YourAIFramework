...
namespace YourAIFramework.ModuleDemos
{
    ...
    public static class DemoServices
    {
        ...
        public static IEnumerator Net()
        {
            ...
            // 所以演示自己用 Time.deltaTime 喂——这正是"泵语义"：谁驱动、喂多少，宿主说了算。
            float watch = 0f;
        ...
        // ==================================================================
        // Demo 16：AI（可选）——无 Key 照常演示"失败是值"；配了 Key 就是真对话
        // ==================================================================
        public static IEnumerator Ai()
        {
            ...
            catch (System.Exception ex)
            {
                ...
                Debug.Log("[Demo16] AI 服务创建失败（失败是值）: " + ex.Message);
                yield break;
            }

            // 发起一回合：泵驱动 —— BeginTurn 非阻塞，每帧 Pump 推进流式输出。
            ...
            {
                Debug.Log("[Demo16] 对话失败（失败是值，原因真实可读）: " + result.Error
