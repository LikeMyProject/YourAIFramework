...
namespace YourAIFramework.ModuleDemos
{
    ...
    /// 可复现随机 → 属性表三层求值 → 泵驱动效果 → 伤害管道。
    ///
    /// 这一整套的价值不在"能算伤害"，而在**每一层都各有一个可替换的接缝**：
    ...
    internal static class DemoStats
    {
        ...
        public static IEnumerator Stats()
        {
            ...
            Log("    读未声明的 magic → TryValue 返回 " + declared + "、值 " + readBack
                + "（读松：配表驱动的属性集合本来就会动态长出来；而写侧拼错属性名会立刻抛异常）。");

            yield return null;

            // ---------------------------------------------------------- 3. 泵驱动效果
            Log("③ 持续效果 —— 泵驱动的状态机：施加/叠层/跳伤/到期都是 Pump(delta) 推出来的。");
