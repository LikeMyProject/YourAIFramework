...
namespace YourAIFramework.ModuleDemos
{
    ...
    ///
    /// - Start：自动依次上演 20 个模块演示（每个之间停 2 秒方便看 Console）；
    ...
    public sealed class DemosHub : MonoBehaviour
    {
        private IEnumerator Start()
        {
            ...
            Debug.Log("<color=#4FC3F7>  Your AI Framework —— 20 个模块演示开始（Console 逐行上演）</color>");
            Debug.Log("<color=#4FC3F7>=================================================================</color>");

            yield return Tour("01/20 模块中心 ModuleCenter —— 一切皆模块，依赖就是数字", DemoCore.ModuleCenter());
            yield return Tour("02/20 事件总线 EventBus —— Publish 同步直发 / Post + Pump 延迟派发", DemoCore.EventBus());
            yield return Tour("03/20 对象池 ObjectPool / GameObjectPool —— 借还配平，失衡即抛", DemoCore.ObjectPools());
            yield return Tour("04/20 存档与设置 SaveSystem / SettingStore —— 原子写 + 键值设置", DemoContent.SaveAndSettings());
            yield return Tour("05/20 流程状态机 ProcedureFlow —— Boot→Menu→Playing", DemoContent.ProcedureFlowDemo());
            yield return Tour("06/20 数据表 DataTable —— 配表读取/点路径/格式化/整表替换", DemoContent.DataTableDemo());
            yield return Tour("07/20 资源 AssetManager —— 统一入口轮询加载，失败是值", DemoContent.AssetsDemo());
            yield return Tour("08/20 本地化 LocalizationStore —— 多语言/回退链/格式化", DemoContent.LocalizationDemo());
            yield return Tour("09/20 UI 面板栈 PanelStack + UIPanelManager —— 层级与独占屏蔽", DemoExperience.UiPanels());
            yield return Tour("10/20 声音 AudioManager —— 分组混音/优先级抢断/实时静音", DemoExperience.Audio());
            yield return Tour("11/20 实体 EntityRegistry —— 小接口自由组合，不强迫继承", DemoExperience.Entities());
            yield return Tour("12/20 网络 NetClient —— 心跳/退避重连/断线排队（演示域名必失败，看失败路径）", DemoServices.Net());
            yield return Tour("13/20 SDK 管道 SdkHub —— 渠道故障自动降级", DemoServices.Sdk());
            yield return Tour("14/20 诊断台 FrameworkDiagnostics —— 计数器/仪表/一键快照", DemoServices.Diagnostics());
            yield return Tour("15/20 热更编排 HotUpdateFlow —— 阶段编排/失败截停/终态纪律", DemoServices.HotUpdate());
            yield return Tour("16/20 AI 模块（可选）—— 无 Key 一切照常，配 Key 即真对话", DemoServices.Ai());
            yield return Tour("17/20 自研资源包 AssetPack —— 清单/版本差量/下载队列/依赖闭包", DemoPack.AssetPack());
            yield return Tour("18/20 自研本地化包 LocalePack —— 语言链/多表共享/CLDR 复数/内联格式化/资源地址", DemoLocale.LocalePack());
            yield return Tour("19/20 自研热更程序集 AssemblyReload —— 清单/拓扑装载/完整性校验/入口点/AOT 白名单", DemoHotAssembly.HotAssemblies());
            yield return Tour("20/20 数值系统 Stats —— 可复现随机/属性表三层求值/泵驱动效果/伤害管道", DemoStats.Stats());
