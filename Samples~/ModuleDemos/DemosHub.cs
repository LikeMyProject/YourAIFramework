...
namespace YourAIFramework.ModuleDemos
{
    ...
    ///
    /// - Start：自动依次上演 16 个模块演示（每个之间停 2 秒方便看 Console）；
    ...
    public sealed class DemosHub : MonoBehaviour
    {
        private IEnumerator Start()
        {
            ...
            Debug.Log("<color=#4FC3F7>  Your AI Framework —— 16 个模块演示开始（Console 逐行上演）</color>");
            Debug.Log("<color=#4FC3F7>=================================================================</color>");

            yield return Tour("01/16 模块中心 ModuleCenter —— 一切皆模块，依赖就是数字", DemoCore.ModuleCenter());
            yield return Tour("02/16 事件总线 EventBus —— Publish 同步直发 / Post + Pump 延迟派发", DemoCore.EventBus());
            yield return Tour("03/16 对象池 ObjectPool / GameObjectPool —— 借还配平，失衡即抛", DemoCore.ObjectPools());
            yield return Tour("04/16 存档与设置 SaveSystem / SettingStore —— 原子写 + 键值设置", DemoContent.SaveAndSettings());
            yield return Tour("05/16 流程状态机 ProcedureFlow —— Boot→Menu→Playing", DemoContent.ProcedureFlowDemo());
            yield return Tour("06/16 数据表 DataTable —— 配表读取/点路径/格式化/整表替换", DemoContent.DataTableDemo());
            yield return Tour("07/16 资源 AssetManager —— 统一入口轮询加载，失败是值", DemoContent.AssetsDemo());
            yield return Tour("08/16 本地化 LocalizationStore —— 多语言/回退链/格式化", DemoContent.LocalizationDemo());
            yield return Tour("09/16 UI 面板栈 PanelStack + UIPanelManager —— 层级与独占屏蔽", DemoExperience.UiPanels());
            yield return Tour("10/16 声音 AudioManager —— 分组混音/优先级抢断/实时静音", DemoExperience.Audio());
            yield return Tour("11/16 实体 EntityRegistry —— 小接口自由组合，不强迫继承", DemoExperience.Entities());
            yield return Tour("12/16 网络 NetClient —— 心跳/退避重连/断线排队（演示域名必失败，看失败路径）", DemoServices.Net());
            yield return Tour("13/16 SDK 管道 SdkHub —— 渠道故障自动降级", DemoServices.Sdk());
            yield return Tour("14/16 诊断台 FrameworkDiagnostics —— 计数器/仪表/一键快照", DemoServices.Diagnostics());
            yield return Tour("15/16 热更编排 HotUpdateFlow —— 阶段编排/失败截停/终态纪律", DemoServices.HotUpdate());
            yield return Tour("16/16 AI 模块（可选）—— 无 Key 一切照常，配 Key 即真对话", DemoServices.Ai());
