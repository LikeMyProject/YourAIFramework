...
namespace YourAIFramework.ModuleDemos
{
    ...
    public sealed class DemosHub : MonoBehaviour
    {
        private IEnumerator Start()
        {
            ...
            // 自检：模块的 Init 前置条件是「运行期」的，四套离线检查都看不见 ——
            ...
            yield return Tour("07/23 资源 AssetManager —— 统一入口轮询加载，失败也是值", DemoContent.AssetsDemo());
            ...
            yield return Tour("17/23 自研资源包 AssetPack —— 清单/版本差量/下载队列/依赖全链", DemoPack.AssetPack());
            ...
            yield return Tour("20/23 数值系统 Stats —— 可复现随机/属性表三层求值/逐帧驱动效果/伤害管道", DemoStats.Stats());
            ...
            yield return Tour("23/23 技能 Skills —— 配置驱动定义/地址解析/目标选择/冷却与吟唱/数值桥落地", DemoSkill.Skills());
