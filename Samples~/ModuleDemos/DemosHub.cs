...
namespace YourAIFramework.ModuleDemos
{
    ...
    ///
    /// - Start：自动依次上演 22 个模块演示（每个之间停 2 秒方便看 Console）；
    ...
    public sealed class DemosHub : MonoBehaviour
    {
        private IEnumerator Start()
        {
            ...
            Debug.Log("<color=#4FC3F7>  Your AI Framework —— 22 个模块演示开始（Console 逐行上演）</color>");
            Debug.Log("<color=#4FC3F7>=================================================================</color>");
            ...
            yield return Tour("21/22 地址目录表 AddressCatalog —— 变体解析/补丁合并/校验/确定性写出", DemoCatalog.AddressCatalogDemo());
            yield return Tour("22/22 脚本层 Script/XLua —— 清单/装载序/沙箱策略/失败真回滚（IL2CPP 热更代码）", DemoScript.ScriptLayer());
