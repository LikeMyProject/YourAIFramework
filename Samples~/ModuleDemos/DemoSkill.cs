...
namespace YourAIFramework.ModuleDemos
{
    ...
    internal static class DemoSkill
    {
        ...
        /// 由项目自己的实体系统回答 —— 这是技能与"谁在打架"之间**唯一**的预留接口。
        /// </summary>
        ...
        {
            return tags.Count == 0 ? "标准档（不带标签）" : "{" + string.Join(",", tags.Tags.ToArray()) + "}";
        }
        ...
        public static IEnumerator Skills()
        {
            ...
            Log("  同一份技能表，换成" + Describe(standard) + " → "
                + catalog.Resolve("skill/ember/vfx", standard).Locator
            ...
            Log("演示完毕：配置驱动定义、目录表解析地址、纯函数选目标、逐帧驱动冷却与吟唱、"
