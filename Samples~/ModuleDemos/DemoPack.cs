...
{
    /// <summary>
    /// Demo17：自研资源包 —— 一次演示走完"清单 → 版本差量 → 下载队列 → 依赖闭包装载"。
    ///
    ...
    internal static class DemoPack
    {
        ...
        /// <summary>内存版下载后端：两次轮询把字节"落盘"，第二次报 Done。</summary>
        ...
        public static IEnumerator AssetPack()
        {
            ...
            {
                Debug.LogWarning("[Demo17] 下载有失败（失败是值，带原因）: " + queue.FirstFailureReason);
            }
            else
            {
                Debug.Log("[Demo17] 下载完成：" + queue.DoneCount + "/" + queue.Tasks.Count + " 个 bundle，"
                    + queue.ReceivedBytes + " 字节（真实后端此时会做 CRC 校验，并把结果记进缓存账本）");
            }

            // ---- 4. 依赖闭包装载：一个地址 → 拓扑序装载整条闭包 → 取出资源 ----
            ...
            {
                Debug.LogWarning("[Demo17] 装载失败（失败是值，原因原文）: " + request.Error);
            }
