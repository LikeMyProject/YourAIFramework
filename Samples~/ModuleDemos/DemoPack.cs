...
namespace YourAIFramework.ModuleDemos
{
    ...
    internal static class DemoPack
    {
        ...
        private sealed class InstantDownloadBackend : IDownloadBackend
        {
            ...
            /// <summary>替身也得说真话：按清单声明的大小上报，不然它报的字节数和清单对不上。</summary>
            ...
            public object Begin(string url, string savePath, long resumeFrom, out string error)
            {
                ...
                // url 最后一段就是 bundle 名（Enqueue 时拼的 "https://cdn.example.com/<bundle>"）
                string bundle = url.Substring(url.LastIndexOf('/') + 1);
                long size;
                if (!Sizes.TryGetValue(bundle, out size))
                {
                    size = 2048;    // 清单里没有声明的，才落回这个占位数
                }

                _live[handle] = new Attempt { Size = size, Polls = 0 };
                return handle;
            }
        ...
        public static IEnumerator AssetPack()
        {
            ...
            InstantDownloadBackend transport = new InstantDownloadBackend();
            for (int i = 0; i < manifest.Bundles.Count; i++)
            {
                transport.Sizes[manifest.Bundles[i].Name] = manifest.Bundles[i].Size;
            }

            DownloadQueue queue = new DownloadQueue(transport);
