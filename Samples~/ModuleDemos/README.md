...
> SDK 渠道必须在 `SdkHub.Init` 之前注册。这类检查是**运行期**的 —— 四道离线门禁只看编译与
> 纯 C# 断言，看不见它们，缺了会在 Play 时当场抛异常。改动注册清单时，对照 `DemoSetup`
...
| 20 | 数值系统 Stats | 确定性随机 → 属性表三层求值 → 泵驱动效果 → 伤害管道 | 同种子同序列（分流互不扰动）；3 秒 DoT 正好跳 3 次；攻方缺攻击属性 → 失败是值 |
