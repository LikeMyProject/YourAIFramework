...
**同一个道理还有另一半：模块的"启动那一下"（Init）也不会自己发生。** `ProcedureFlow` 的
`Start<T>()` 只是记下"起点是谁"，真正进入第一个状态在 `Init` 里——注册成模块时由
`GameEntry` 调，局部实例要自己调 `flow.Init(null)`（离线检查项也是这么写的）。少了它
不会报错，而是**什么都没演就结束**、`State` 一直是 `Idle`（`DemoContent.ProcedureFlowDemo`
里有正例和注释）。
