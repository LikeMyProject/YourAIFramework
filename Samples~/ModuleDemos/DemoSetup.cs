...
namespace YourAIFramework.ModuleDemos
{
    ...
    /// 3. 按 Play——Console 会依次上演全部 23 个模块演示。
    ///
    /// 注册顺序就是依赖关系：InitOrder 小的先初始化，数字摆在明面上。
    ///
    /// **注册的模块都要满足各自的 Init 前置条件**（fail-fast —— 缺了就在 Play 当场抛）：
    ///
    /// | 模块 | 前置条件 |
    /// |---|---|
    /// | `ProcedureFlow` | 必须 `Add(...)` + `Start<T>()` 设起始状态（这里由 DemoIdleProcedure 补上） |
    /// | `SdkHub` | 渠道必须在 Init **之前**注册（见 DemosServices.RegisterSdkChannels） |
    /// | `AudioManager` / `UIPanelManager` / `AssetManager` | 构造时把后端 / PanelSettings / Provider 传齐 |
    /// | `HotUpdateFlow` | 只登记监听器，阶段在 Demo 15 里现组 |
    ///
    /// 这些前提是**运行期**的：四套离线检查只覆盖编译与纯 C# 检查项，看不见它们。
    /// 所以改动本文件的注册清单时，请对着这张表过一遍。
    /// </summary>
    public static class DemoSetup
    {
        ...
        public static void RegisterAll()
        {
            ...
            // 流程机是 fail-fast 的：一个没有起点的流程机就是接错线，Init 会直接抛
            // （"no start procedure set"）。演示不绕开这条规则，而是给它一个最小的真状态；
            // 完整的状态迁移教学在 Demo 05（那边用独立实例演一遍 Boot → Menu → Playing）。
            Flow.Add(new DemoIdleProcedure()).Start<DemoIdleProcedure>();
        ...
        /// 真实项目这里换成从 AssetManager 取 AudioClip（自研资源包）。
        /// </summary>
    ...
    /// <summary>
    /// 演示常驻状态：给注册进 GameEntry 的流程机一个合法起点。
    ///
    /// 它刻意什么都不做 —— 不抢 Demo 05 的戏（那边用独立实例演完整的 Boot → Menu → Playing），
    /// 这里只负责"流程机是起着的"，让 GameEntry 里的模块栈与真实项目同构。
    /// </summary>
    internal sealed class DemoIdleProcedure : Procedure
    {
        public override string Name { get { return "DemoIdle"; } }

        public override void OnEnter()
        {
            Debug.Log("[DemoSetup] 流程机已进入 DemoIdle（演示常驻状态；完整迁移见 Demo 05）");
        }
    }
