...
namespace YourAIFramework.ModuleDemos
{
    ...
    internal static class DemoHotAssembly
    {
        ...
        {
            // 清单里的 crc 必须是这四段字节**真算出来的**值，不能写占位值：
            // 装载时先比长度、再比 crc32，占位值会让 Core 在第一帧就被完整性校验打回，
            // 于是后面三段（逐帧装载成功 / 长度不符 / 装载器拒绝）全都演不出来 ——
            // 全都变成同一个 crc 报错。这几个数就是 Crc32.Compute({1,2,3,4}) 之类的结果。
            const string ManifestJson =
                "{\"version\":\"1.2.0\",\"assemblies\":["
                + "{\"name\":\"Core\",\"fileName\":\"Core.dll.bytes\",\"size\":4,\"crc\":3057449933,"
                + "\"hash\":\"h-core\"},"
                + "{\"name\":\"Game\",\"fileName\":\"Game.dll.bytes\",\"size\":4,\"crc\":1401769321,"
                + "\"dependencies\":[\"Core\"]},"
                + "{\"name\":\"UI\",\"fileName\":\"UI.dll.bytes\",\"size\":4,\"crc\":3147399328,"
                + "\"dependencies\":[\"Core\"]},"
                + "{\"name\":\"Hot\",\"fileName\":\"hot/main.dll.bytes\",\"size\":4,\"crc\":1134503520,"
