...
namespace YourAIFramework.ModuleDemos
{
    ...
    public static class DemoContent
    {
        ...
        public static IEnumerator SaveAndSettings()
        {
            ...
            string saveError = DemoSetup.Saves.Save("slot1", payload, version: 1);
            Debug.Log("[Demo04] 已写入存档 slot1："
                + (saveError == null ? "成功" : "失败 → " + saveError)
                + "（Save 成功时返回 null、失败才带原因 —— 这就是「失败也是值」的写法）；存档目录 "
                + DemoSetup.Saves.DirectoryPath);
            ...
            settings.SaveToDisk();
            Debug.Log("[Demo04] 设置已写进文件: music.volume=" + settings.GetInt("music.volume", 50)
        ...
        public static IEnumerator ProcedureFlowDemo()
        {
            ...
            // Start<T>() 只记下"起点是谁"，真正把它跑起来的是 Init —— 注册成模块时由
            // GameEntry 按 InitOrder 调，本地 new 的实例得自己调一次（离线检查项也是这么起的）。
            // 少了这一下，State 就一直是 Idle：下面的 while 一次都不会进，演示"什么都没演就结束"。
            flow.Init(null);

            Debug.Log("[Demo05] 启动流程 Boot → Menu → Playing，每态停留约 1 秒：");
            int frames = 0;
            while (flow.State == ProcedureFlowState.Running && frames < 600)
            {
                flow.Pump();                    // 本地实例自己泵；注册成模块时 GameEntry 每帧替你泵
                frames++;
                yield return null;
            }

            if (flow.State == ProcedureFlowState.Running)
            {
                Debug.LogWarning("[Demo05] 流程机超过 " + frames + " 帧仍在 Running —— "
                    + "正常应当 3 秒左右自己走完，没走完说明状态迁移卡住了。");
            }
        ...
        public static IEnumerator AssetsDemo()
        {
            ...
            // 加载不会自己往前走：必须有人每帧调一次 Pump（这就是"谁驱动、何时驱动由宿主决定"）。
            // 这里的 AssetManager 是本地实例，不在 ModuleCenter 里，所以自己泵；
            // 注册进 GameEntry 的实例由 GameEntry 的 Update 统一泵（见 DemoSetup.Assets）。
            // 上限 120 帧只为"万一没落定也别死等"，正常情况下面两个 Provider 第一帧就落定。
            int frames = 0;
            while ((!fromMemory.IsDone || !fromFile.IsDone) && frames < 120)
            {
                assets.Pump();
                frames++;
                yield return null;
            ...
            if (!fromMemory.IsDone || !fromFile.IsDone)
            {
                Debug.LogWarning("[Demo07] 超过 " + frames + " 帧还没落定：内存=" + fromMemory.State
                    + "、文件=" + fromFile.State + " —— 两个 Provider 都该在第一次 Pump 就落定，"
                    + "没落定说明它们没调 Complete/Fail。");
                yield break;
            }
            ...
            // 失败也是值：不存在的 key 不抛异常，落 Failed + 原因。
            AssetRequest missing = assets.Load("no/such/file.json");
            frames = 0;
            while (!missing.IsDone && frames < 120)
            {
                assets.Pump();
                frames++;
                yield return null;
