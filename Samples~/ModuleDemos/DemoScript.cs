...
namespace YourAIFramework.ModuleDemos
{
    ...
    ///   - 内核（本演示用的这一层）只定义预留接口：代码块、清单、装载序、沙箱策略、失败时的处理。
    ///     它**不认识任何脚本引擎**，也不依赖任何第三方包；
    ///   - 真正的解释器是适配层：<c>Runtime/YourFramework.XLua</c> 把 XLua 接到这套预留接口上
    ///     （需要用户导入 XLua 并打开 <c>YOURFRAMEWORK_XLUA</c> 宏）。
    ///
    /// 本演示跑的是**玩具运行时**：不解释 Lua，只按 <c>function 名字</c> 登记入口，
    /// 好让「预留接口怎么用、失败怎么折、回滚长什么样」在离线环境里能被看见。
    /// 换成 XLua 适配层时，下面这段演示代码一个字都不用改 —— 这正是预留接口的意义。
    /// </summary>
  ...
  ""chunks"": [
    ...
    { ""name"": ""ui.hud"", ""fileName"": ""ui/hud.lua"", ""entries"": [""YourFramework_Hud""] }
        ...
        public static IEnumerator ScriptLayer()
        {
            ...
            int applyFrames = 0;
            while (apply.Pump() == HotUpdateStepState.Running && applyFrames < 1000)
            {
                runtime.Pump(0.016d); // 每帧滴答一下解释器：泵哲学，不靠后台线程
                applyFrames++;
            }

            if (apply.State == ScriptPatchState.Loading)
            {
                Log("  装载超过 1000 步仍未到终态 —— 每个阶段都该自己推进到成功/失败，这是死循环，先查阶段实现。");
            }
            ...
            int failFrames = 0;
            while (failStep.Pump() == HotUpdateStepState.Running && failFrames < 1000)
            {
                failFrames++;   // 泵到终态
            }

            if (failStep.State == ScriptPatchState.Loading)
            {
                Log("  失败注入超过 1000 步仍未到终态 —— 该阶段应当失败并停下，没停就是死循环。");
            }
