...
namespace YourFramework.XLua
{
    ...
    ///    所以 loader 只认我们自己的代码块来源，并套同一道路径闸（拒绝绝对路径与「..」）。
    /// 3. **关掉不该有的门** → 见 <see cref="XLuaSandbox"/>：策略来自内核，
    ///    这里只负责把「哪些全局该消失」翻译成 Lua 能执行的东西，并**回读确认真的消失了**。
    /// 4. **滴答** → <c>LuaEnv.Tick()</c>。泵驱动的框架不靠后台线程，帧循环里滴答一下即可。
    ///
    /// 错误姿态严格照抄内核合约：
    /// - 语法错、入口抛异常、块缺席 → 失败是值，带解释器自己的原文（排查靠的就是那句原文）；
    ...
    public sealed class XLuaScriptRuntime : IScriptRuntime, IDisposable
    {
        ...
        /// 拿到它就意味着绕过了本类的守卫，所以只在环境就绪时非 null。
        /// </summary>
        ...
        /// 所以这里和文件来源套同一道闸：拒绝绝对路径、拒绝「..」。找不到返回 null，
