...
namespace YourFramework.Event
{
    ...
    ///
    /// 处理器抛异常：异常**照原样抛给发布方，不吞**，但总线自己的记账在 finally 里复原 ——
    /// 一次异常不会让退订失效、压实停摆或 Clear() 永久不可用。代价是这一次派发里排在
    /// 抛异常者后面的订阅者收不到该事件；若要"一个订阅者坏掉不影响其余"，处理器自己
    /// 兜住即可，或把事件放到 ModuleCenter 的模块泵上（那里是逐模块隔离的）。
    /// </summary>
    public sealed class EventBus
    {
        ...
        // Every subscriber list currently on the dispatch stack, outermost first.
        // A stack rather than "the one list being dispatched" because nesting is legal:
        // with a single slot, unsubscribing during a nested publish saw the *inner*
        // list, decided it was not iterating, and removed from the outer list while
        // the outer loop was still walking it -- index shifted, handlers skipped.
        // Depth is 1 in the common case, so the linear membership test is free.
        private readonly List<List<Delegate>> _dispatching = new List<List<Delegate>>();
        ...
        private void BeginDispatch(List<Delegate> list)
        {
            _dispatching.Add(list);
        }

        /// <summary>
        /// Leaves the innermost dispatch. In a finally, because a throwing handler must
        /// not strand the bookkeeping: a stranded stack used to mean Unsubscribe stopped
        /// really removing, dirty-list compaction never ran again, and Clear() threw for
