...
namespace YourFramework.Core
{
    ...
    ///    有意义，规则一句话讲完。
    ///
    /// 错误姿态：注册/生命周期时序错误是程序 bug，直接抛 InvalidOperationException
    /// （fail-fast）；模块自身的运行期错误由模块自己按“失败是值”处理。
    /// </summary>
    public sealed class ModuleCenter
    {
        ...
        /// <summary>
        /// Shuts every module down in reverse init order. Idempotent: a second call
        /// is a no-op, and modules whose Init threw are skipped gracefully.
        /// </summary>
    ...
    public static class AiDiagnosticsHost
    {
        ...
        {
            string line = "[YourFramework] module '" + moduleName + "' threw during " + phase
