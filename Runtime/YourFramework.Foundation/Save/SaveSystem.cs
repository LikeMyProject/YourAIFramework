...
namespace YourFramework.Save
{
    ...
    /// 信封与磁盘纪律：
    ///
    /// - 原子写：新档先整个写进 .tmp，再用操作系统的原子替换顶上当前档，
    ///   旧档顺势落进 .sav.1。进程被杀只会看到“全是旧档”或“全是新档”，
    ///   不存在“没有档”或“半截档”的中间态；
    /// - 备份轮转：每次 Save 把上一版挪进 .sav.1 ... .sav.{keepBackups}；
    /// - 损坏隔离：读不懂的存档改名 .corrupt 让路（与 FileMemoryStore 同一判断），
    ///   游戏按“存档不存在”处理，玩家还能进新档；
    /// - 失败也是值：磁盘故障不抛异常。TryLoad 返回 false、Save/Load 类方法把错误
    ///   原因作为返回值带回，让 UI 有话可说。
    ///
    ...
    public sealed class SaveSystem
    {
        ...
        /// <summary>写一个槽。返回 null 表示成功，否则是错误说明（失败也是值）。</summary>
        public string Save(string slot, JsonValue payload, int version = 1)
        {
            ...
            try
            {
                ...
                // 1) 完整的新档先整个落地。到这里为止，一个既有文件都还没动过 ——
                //    这一步失败，盘上的旧档完好如初。
                File.WriteAllText(tmp, envelope.ToJson());

                // 2) 把备份位往后挪一格，给 .1 腾地方（当前档不动）。
                RotateBackups(slot);

                // 3) 原子替换：成功则新档上任、旧档落进 .1；失败则旧档原封不动。
                bool keepBackup = _keepBackups > 0;
                AtomicFile.Replace(tmp, path, keepBackup ? BackupPath(slot, 1) : null);
                return null;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 备份位整体后挪一格（丢最老的），把 .1 腾空给 <see cref="AtomicFile.Replace"/>。
        ///
        /// **不碰当前档**：在当前档被原子替换掉之前，它必须原封不动地待在原地。
        /// 挪它 -> 写新档 -> 覆盖回来这条路，中间有一段盘上无档可读的窗口，
        /// 进程恰好死在那儿，就是“两份完好数据都在盘上，框架一份也不认”。
        /// </summary>
        private void RotateBackups(string slot)
        {
            if (_keepBackups <= 0)
            {
                return;
            ...
            DeleteIfExists(BackupPath(slot, _keepBackups));

            for (int i = _keepBackups - 1; i >= 1; i--)
            {
                string from = BackupPath(slot, i);
                if (File.Exists(from))
                {
                    File.Move(from, BackupPath(slot, i + 1));
                }
            }
        }
