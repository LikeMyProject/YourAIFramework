...
namespace YourFramework.Scripting
{
    ...
    ///
    /// 失败也是值：读不到就返回 false + 原因，不抛异常 —— 磁盘 IO 与网络 IO 同级，都是运行期故障。
    /// 与程序集字节来源（IAssemblyBytesSource）是同一副处理方式，只是这里读的是**文本**：
    ...
    /// 默认来源：缓存目录下的相对路径。
    ///
    /// 路径纪律与热更程序集共用同一道检查（<see cref="RelativePathGuard"/>）：拒绝绝对路径、
