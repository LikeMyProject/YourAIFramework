...
namespace YourFramework.Assets
{
    ...
    /// (YooAsset adapter, downloaders). Contract:
    ///
    ...
    ///
    /// 设计取舍：
    /// 1. 自家接口在前、引擎后端在后 —— IAssetProvider 是唯一扩展点，YooAsset /
    ///    Addressables / 编辑器直读都只是 Provider 实现，游戏代码永远面对本类；
    /// 2. 轮询泵语义 —— 没有 Task/回调风暴，Pump 每帧推进一帧的量，与 ModuleCenter、
    ///    AI 层同一哲学，谁驱动、何时驱动宿主说了算；
    /// 3. 失败是值 —— 加载失败落 Failed + Error 原因，不抛异常，UI 有话可说；
    ...
    public sealed class AssetManager : IModule
    {
        ...
        public void Clear()
        {
            ...
            {
                _inFlight[i].Fail("cleared while loading");
                _byKey.Remove(_inFlight[i].Key);
            }
