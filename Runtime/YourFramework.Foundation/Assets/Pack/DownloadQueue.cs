...
namespace YourFramework.Assets.Pack
{
    ...
    /// <summary>
    /// 下载队列：并发上限 + 指数退避重试 + 超时截停 + 断点续传，泵驱动。
    ///
    /// 设计取舍：
    /// 1. 语义对齐框架红线 —— 没有 async/await，每帧推进一帧的量；时间由调用方
    ///    用 deltaSeconds 注入，因此超时/退避可在无头环境里被精确断言；
    /// 2. 失败是值 —— 每次尝试失败只是"这次没成"，连败到 MaxAttempts 才落 Failed
    ///    并把最后一次原因原文带出来；队列永远不抛（除了接线 bug：空 URL、重名任务）；
    /// 3. 断点续传靠"后端报告已落盘字节数"这一个事实源：队列把已落盘字节作为下次
    ///    尝试的 resumeFrom，不再自作聪明地拼接偏移；
