...
namespace YourFramework.HotUpdate
{
    ...
    ///
    /// 引入不自研的落点（公子拍板）：YooAsset / HybridCLR 的能力以 IHotUpdateStep
    /// 的形态插入（引擎侧适配层负责把它们的异步轮询成帧），本类只做编排 ——
    /// 顺序、进度事件、失败截停、终态语义。纯 C#，mock 步骤离线全测。
    ///
    ...
    public sealed class HotUpdateFlow : IModule
    {
        ...
        public void Pump()
        {
            ...
            {
                Fail(step.Name, "step '" + step.Name + "' failed");
            }
