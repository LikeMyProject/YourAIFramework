...
namespace YourFramework.Entity
{
    ...
    /// <summary>
    /// 实体注册表：NPC/机关/子弹/系统组件的统一管理 —— 进门 OnSpawn、每帧
    ...
    public sealed class EntityRegistry : IModule
    {
        ...
        ///
        /// 一个实体的 EntityPump 里若 Despawn 了别的实体，列表会当场缩短：按号遍历时
        /// 紧跟其后的那一位会被整个跳过（一帧的位移/计时就丢了）。所以先记住当下这位是谁，
        /// 摘人之后回头核对下标是否还指着它，不是就退一格。
        /// </summary>
        public void Pump(float deltaSeconds)
        {
            for (int i = 0; i < _entities.Count; i++)
            {
                ...
                IEntityTickable tickable = entity as IEntityTickable;
                if (tickable == null)
                {
                    continue;
                }
                ...
                {
                    AiDiagnosticsHost.ReportModuleError("EntityRegistry", "tick '" + entity.Name + "'", ex);
                }

                // 这一拍里有人（含它自己）被摘掉了：下标已经指到别人身上，退一格补回来。
                if (i < _entities.Count && !ReferenceEquals(_entities[i], entity))
                {
                    i--;
                }
            }
        ...
        public void Shutdown()
        {
            ...
            {
                // 先抓住当下这位：OnDespawn 里若顺势 Despawn 了别人，列表会缩短，
                // 回头再按号去摸 _entities[i] 要么摸错人、要么直接越界。
                IEntity entity = _entities[i];
                IDespawnable despawnable = entity as IDespawnable;
                if (despawnable != null)
                {
                    ...
                    catch (Exception ex)
                    {
                        AiDiagnosticsHost.ReportModuleError(
                            "EntityRegistry", "despawn '" + entity.Name + "'", ex);
                    }
                }
            }
