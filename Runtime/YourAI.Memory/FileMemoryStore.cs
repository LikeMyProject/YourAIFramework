...
namespace YourAI.Memory
{
    ...
    ///
    /// One exception to "carry on": when a file exists but could not be read *and*
    /// could not be copied aside either, the store stops writing instead of overwriting
    /// a file whose contents it never saw. Losing this session's memory is recoverable;
    /// silently replacing the player's history with a shorter one is not.
    /// </summary>
    public sealed class FileMemoryStore : IMemoryStore
    {
        ...
        /// <summary>
        /// Set when <see cref="Load"/> could not establish what is on disk and could not
        /// preserve a copy of it either. <see cref="Save"/> then refuses to write, because
        /// a memory file we failed to read is not an empty memory file -- overwriting it
        /// would destroy the only record of what the player had.
        /// </summary>
        ...
        ///
        /// A file that exists and cannot be read at all (held open by another process,
        /// permissions changed) is the third case, and the nastiest one: we do not know
        /// what it holds, and a copy may not be possible either. When no copy could be
        /// preserved, saving is blocked until a later load succeeds -- otherwise one
        /// transient read failure would cost the player their whole history at the next
        /// save point.
        /// </summary>
        public int Load()
        {
            ...
            {
                // 内容读进来了、判得出来是坏的：备份成功就可以放心往下写。
                _writeBlocked = !Quarantine("unparseable: " + parseError);
                return 0;
            }
            ...
            if (!records.IsArray)
            {
                _writeBlocked = !Quarantine("no records array");
                return 0;
            }
        ...
        ///
        /// The write goes to a sibling <c>.tmp</c> file first, which is then swapped in
        /// with the OS-level replace. Both halves matter: the temp file keeps a crash
        /// from leaving a truncated save, and the atomic swap keeps it from leaving a
        /// half-old/half-new one. <c>File.Copy</c> truncates the target and then writes
        /// it byte by byte, so it is not a replace -- a crash between those two points
        /// used to cost the whole file.
        ///
        /// A store whose last <see cref="Load"/> could not read the file refuses to
        /// write unless <paramref name="force"/> says otherwise; see <see cref="Load"/>.
        /// </summary>
        public bool Save(bool force = false)
        {
            ...
            {
                LastError = "Refusing to overwrite " + _path
                    + ": the last load could not read it, so its contents are unknown"
        ...
        /// <summary>
        /// Puts <paramref name="tmp"/> in place of <paramref name="path"/> in one step.
        ///
        /// <c>File.Replace</c> is the OS replace primitive (Windows ReplaceFile / Unix
        /// rename): the target is either wholly the old file or wholly the new one, and a
        /// failure leaves it untouched. It also requires the target to exist, hence the
        /// move on first save.
        /// </summary>
        private static void AtomicReplace(string tmp, string path)
        {
            if (File.Exists(path))
            {
        ...
        /// </summary>
        private bool Quarantine(string reason)
        {
